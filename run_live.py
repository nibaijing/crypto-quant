...
3. 执行模拟下单
4. 每5分钟生成看板 HTML
5. 每30分钟通过 Telegram 推送看板截图

架构:
    K线数据 → 策略引擎 → 信号 → 执行器 → 模拟成交 → 看板
                                                      ↓
                                                 Telegram推送
"""
...
from monitor.dashboard import save_dashboard, generate_dashboard_html
...
root_logger.handlers.clear()  # 清除其他模块添加的 handler
...
# 全局状态
executor: Optional[FuturesExecutor] = None
...
def build_bar_data(df: pd.DataFrame, index: int) -> Dict:
    ...
    bar = {
        ...
        "index": index,
        "position": None,  # 实盘时用 executor.position
    }
...
def process_tick():
    ...
    try:
        ...
        executor.update_bars_held()  # 每根 K 线计数 +1
    ...
    if signal and signal != "HOLD" and signal is not None:
        ...
        if signal == "LONG" and (not executor.position or executor.position.size == 0):
            ...
            if result:
                notify_trade("LONG", close, f"开多仓 {executor._sim_leverage}x")
            ...
            if result:
                notify_trade("SHORT", close, f"开空仓 {executor._sim_leverage}x")
...
def generate_and_push_dashboard(force: bool = False):
    ...
    if force or (now - last_dashboard_time) >= DASHBOARD_INTERVAL:
        ...
        # 不在 run_live.py 里写 dashboard.html，由独立 dashboard server 负责
...
def main():
    ...
    df = fetch_klines_with_history("BTC-USDT", "15m", 200)
    if not df.empty:
    ...
    # 清理
    logger.info("=" * 60)
    logger.info(f"⏹️ 模拟盘已停止 | 运行: {get_uptime()} | 总tick: {tick_count}")
